...
def apply_denoising(image):
    """
    Apply OpenCV's non-local means denoising to reduce noise while preserving edges.
    ...
    """
    # Apply non-local means denoising for color images with light denoising parameters
    return cv2.fastNlMeansDenoisingColored(
        image, 
        None, 
        h=7,  # Light denoising strength
        hColor=10,  # Light denoising strength for color components
        templateWindowSize=5,
        searchWindowSize=21,
    )

def apply_denosing_second_pass(image):
    """
    Apply a second pass of denoising to the image to further reduce noise.
    ...
    numpy.ndarray: Denoised image
    """
    # Apply non-local means denoising with stronger parameters for the second pass
    return cv2.fastNlMeansDenoisingColored(
        image, 
        None, 
        h=5,  # Stronger denoising strength
        hColor=5,  # Stronger denoising strength for color components
...
def auto_perspective_correction(image):
    ...
    # Apply denoising to the input image to improve feature detection
    denoised_image = apply_denoising(image)
    
    # Try perspective correction methods on the denoised image
    result = try_contour_detection(denoised_image)
    if result is None:
        result = try_hough_lines(denoised_image)
    if result is None:
        # Final fallback - just make it square
        result = make_square(denoised_image)
...
def process_images(input_dir, output_dir):
    ...
    if os.path.exists(output_dir):
        print(f"Clearing contents of output directory: {output_dir}")
    ...
    for i, filename in enumerate(image_files):
        ...
        try:
            ...
            denoised = apply_denosing_second_pass(denoised)
            corrected = auto_perspective_correction(denoised)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fix perspective of images to be perfect squares with 90 degree angles and apply denoising."
    )
